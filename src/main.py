...
# for i in L:  # FIXME gurobipy.GurobiError: Constraint has no bool value (are you trying "lb <= expr <= ub"?)
#     for j in D:
#         model.addConstr(f_i_wq[i] * q_ij_m[i, j, 0] * d_i[i] <= Cap_WL * n_ij[i, j], name="23")

# for i in L:  # FIXME TypeError: 'int' object is not subscriptable
#     for j in D:
...
# for i in L:  # FIXME gurobipy.GurobiError: Constraint has no bool value (are you trying "lb <= expr <= ub"?)
#     for j in D:
#         model.addConstr(f_i_wq[i] * q_ij_m[i, j, 2] * d_i[i] >= omega_LTL * g_ij[i, j], name="27")

# for j in D:  # FIXME KeyError: (0, 10)
#     for z in Z:
#         model.addConstr(quicksum(r_iz[i, z] * q_ij_m[i, j, 0] * d_i[i] for i in L) <= Cap_L * n_jz_LTL[j, z], name="28")

# for j in D:  # FIXME IndexError: index 1 is out of bounds for axis 0 with size 1
#     for z in Z:
#         model.addConstr(
